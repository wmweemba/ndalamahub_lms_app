...
**Last updated:** 2026-07-10 — Phase 05 (loan rate engine rebuild) executed end-to-end on `phase/05-loan-engine-rebuild`: rate basis (`per_annum`/`per_term`/`per_period`) and explicit term unit (`days`/`weeks`/`months`) are now configurable at the product and loan level, schedules regenerate anchored to disbursement instead of application date, early-settled loans mark remaining installments `waived` instead of `paid`-with-zero-amount, and the `~21M` implausible prepayment-interest bug is fixed at its root cause. Suite is now 215/215 (195 + 20 new). See Section 6 for the specific defects this phase closed and Section 7 for architecture notes.
...
- **Testing**: Jest 30 on the server, 215 tests total. 170 are model/utility tests under `server/utils/__tests__/` (133 pre-existing model tests, 17 for `server/utils/tenantScope.js` added in Phase 04, and 20 added in Phase 05 for `annualizeRate`/`termToDays`, the Manifi rate-basis/term-unit shape, waived-settlement bookkeeping, and the accrued-interest fix — instantiate models/call the scoping module directly, using `mongodb-memory-server` only where a real DB lookup is needed). Since Phase 03b, Supertest and `mongodb-memory-server` are also exercised by 45 API-level tests under `server/tests/api/`, which hit the live Express app (`server/app.js`) over HTTP against a real (in-memory) MongoDB — these exercise route handlers, `authorize`/`authorizeMinRole` chains, and per-route tenancy checks. A `test.failing` entry (none currently exist) marks a known, deliberately-unfixed defect the test suite pins rather than hides. No client-side automated tests exist yet.
