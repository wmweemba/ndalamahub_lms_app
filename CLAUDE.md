...
- **Testing**: Jest 30 on the server, 241 tests total. 183 are model/utility tests under `server/utils/__tests__/` (133 pre-existing model tests, 17 for `server/utils/tenantScope.js` added in Phase 04, 20 added in Phase 05 for `annualizeRate`/`termToDays`, the Manifi rate-basis/term-unit shape, waived-settlement bookkeeping, and the accrued-interest fix, 3 added in Phase 06 for the user-deletion policy and the orphan-applicant populate guard, 6 added in Phase 07 for `markOverdueInstallments()` (overdue/in_arrears/defaulted transitions, idempotency, recovery, waived/paid non-transition), and 4 added in Phase 09 for `sendPaymentReminders()` (T-3 window selection, reminderSentAt/overdueNoticeSentAt idempotency) — instantiate models/call the scoping module directly, using `mongodb-memory-server` only where a real DB lookup is needed). Since Phase 03b, Supertest and `mongodb-memory-server` are also exercised by 58 API-level tests under `server/tests/api/`, which hit the live Express app (`server/app.js`) over HTTP against a real (in-memory) MongoDB — these exercise route handlers, `authorize`/`authorizeMinRole` chains, and per-route tenancy checks, including 10 added in Phase 08 (`tickets.test.js`) for handler-company routing, the visibility matrix, status-change authority, and the reply-reopens-a-resolved-ticket behavior, and 3 added in Phase 09 (`email.test.js`) pinning the no-token-in-forgot-password-response contract and the approve-route email wiring (with `utils/email.js` mocked — no real sends in the automated suite). Note: Phase 08's doc specified the ticket tests' path as `server/utils/__tests__/tickets.test.js`, but since they're Supertest/HTTP tests (not unit tests calling models directly), they were placed under `server/tests/api/` instead, consistent with the Phase 03b convention — a deliberate deviation from the doc, not an oversight. A `test.failing` entry (none currently exist) marks a known, deliberately-unfixed defect the test suite pins rather than hides. No client-side automated tests exist yet.
- **Scheduled jobs**: `server/jobs/scheduler.js` runs two daily cron jobs via `node-cron` (Africa/Lusaka), started from `server/server.js` unless `NODE_ENV=test` or `ENABLE_CRON=false`: `markOverdueInstallments()` (`server/jobs/markOverdueInstallments.js`, built Phase 07) at 01:00, and `sendPaymentReminders()` (`server/jobs/sendPaymentReminders.js`, built Phase 09 — T-3-day reminders and overdue notices, both idempotent via `reminderSentAt`/`overdueNoticeSentAt` stamps on the installment) at 08:00. Manual/operator triggers: `pnpm job:overdue` (`server/jobs/runMarkOverdue.js`) and `pnpm job:reminders` (`server/jobs/runSendReminders.js`) — also how the first production backfill/send happens post-deploy. Single-process assumption only; if the Coolify migration ever runs more than one server instance, these jobs need a lock to avoid double-running.
- **Email**: `server/utils/email.js` (`sendEmail`) via Resend — built Phase 09. Never throws; no-ops with a warn log when `RESEND_API_KEY` is unset, so dev/test never need credentials. Templates in `server/utils/emailTemplates.js`. See the Phase 09 entry in Section 2 for what's wired.
- **Exports**: PDFKit, ExcelJS — confirmed working end-to-end (not placeholders)

### Test command — resolved
The placeholder-test-script concern was based on a stale pre-merge snapshot. Post-merge, `server/package.json` has `"test": "jest"` (plus `test:watch`, `test:coverage`), and `cd server && pnpm test` was verified to run the real test suite on 2026-07-04 (133 tests then; 241 as of Phase 09). Treat `pnpm test` in `server/` as the regression gate. Since Phase 03b, `server.js` is a thin bootstrap (dotenv/env fail-fast/`connectDB()`/`app.listen()`); the actual Express app lives in `server/app.js` and is what the API tests import — `pnpm start`/`pnpm dev` behavior is unchanged.
