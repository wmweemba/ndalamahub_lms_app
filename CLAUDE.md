...
**Last updated:** 2026-07-10 — Phase 04 (multi-tenancy scoping layer) executed end-to-end: `server/utils/tenantScope.js` added as the single reusable tenant-scoping module, and `server/routes/{loans,reports,dashboard,users,companies,products,system}.js` migrated off ~30 ad-hoc, per-route company-ObjectId comparisons onto it. Suite is now 195/195 (178 + 17 new `tenantScope.test.js` tests). See Section 7 for the resulting architecture and Section 6 for the audit gaps this phase closed.
...
- Phased execution plans **have been generated**: `docs/01-security-critical-fixes.md` through `docs/10-subscription-gating.md`, indexed in `docs/README.md`, plus `docs/03b-api-test-scaffold.md` (an API/route-level test scaffold and tenant-isolation regression baseline, inserted between 03 and 04). Phases 01–03, 03b, and 04 are executed; 05–10 have not been. Claude Code executing a phase must follow that phase's document exactly — see Section 11 (Working Rules) before touching code.
...
- **Testing**: Jest 30 on the server, 195 tests total. 150 are model/utility tests under `server/utils/__tests__/` (133 pre-existing model tests plus 17 for `server/utils/tenantScope.js`, added in Phase 04 — instantiate models/call the scoping module directly, using `mongodb-memory-server` only where the module does a real Company lookup). Since Phase 03b, Supertest and `mongodb-memory-server` are also exercised by 45 API-level tests under `server/tests/api/`, which hit the live Express app (`server/app.js`) over HTTP against a real (in-memory) MongoDB — these exercise route handlers, `authorize`/`authorizeMinRole` chains, and per-route tenancy checks. A `test.failing` entry (none currently exist) marks a known, deliberately-unfixed defect the test suite pins rather than hides. No client-side automated tests exist yet.
...
Full detail, evidence, and file/line references are in `docs/AUDIT_REPORT.md` — don't duplicate that detail here, just be aware it exists and gates everything else. Phase 01 (register/RBAC/repayment/prepayment fixes plus the settlement-savings fix) and Phase 02 (auth/platform hardening, all items above) are both merged into `main`. All security-baseline items from the audit are now resolved except the cron/scheduler gap (unscheduled).
