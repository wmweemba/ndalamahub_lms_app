...
- **Deploy note (loan-number counter seeding)**: before this code goes live in any environment with existing loan data, seed that environment's `counters` collection for the current year to the current max sequence, or new loan numbers will collide with existing ones. Example (mongosh): find the max existing sequence for the year — `db.loans.find({loanNumber: /^LN2026/}).sort({loanNumber: -1}).limit(1)` — then `db.counters.updateOne({_id: "loanNumber-2026"}, {$set: {seq: <that max suffix as a number>}}, {upsert: true})`. Not run against any live database in this session (see verification note below).
- **Step 9.2 manual verification**: run as an isolated, scripted verification (Supertest + `mongodb-memory-server` against the real Express app, using the existing `server/tests/helpers/fixtures.js`/`tokens.js` harness) rather than against the shared Atlas dev database used in prior phases — no explicit go-ahead was sought/given for this session to write to that shared instance, and the isolated approach gives equivalent confidence without touching external state. Configured the real Manifi product (`flat_rate`, `per_term`, rate 25, `term: {min: 30, max: 30, default: 30, unit: 'days'}`), applied for a 10,000 loan against it as a fresh borrower, approved and disbursed as the lender admin: confirmed `totalInterest` exactly 2500, `totalAmount` exactly 12500, a single installment of 12500 due exactly 30 calendar days after `disbursedAt`, and `endDate` matching that due date. Separately, settled a seeded active loan early via `POST /:id/early-settlement`: confirmed `getSummary().remainingBalance === 0`, `totalPaid` equal to the settlement amount (counted once, not double-counted with installment amounts), `status: 'completed'`, and all remaining installments `waived` (none `paid` with a zero amount). The temporary verification test file was deleted after the run — not part of the committed suite.
