...
- **Not independently verified**: acceptance criterion #3's manual dev-server check (borrower sees Apply button, lender_admin sees company filter with real names, lender_officer sees the lender dashboard) — no Atlas dev-DB credentials available in this session. Flagged for a manual spot-check before the demo.
