...
function BorrowerBottomNav({ navItems }) {
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-[--sidebar] border-t border-[--sidebar-border] flex z-40">
      {navItems.map((item) => (
        <NavLink
          key={item.to}
          to={item.to}
          className={({ isActive }) =>
            `flex-1 flex flex-col items-center justify-center gap-1 py-2 min-h-[44px] ${
              isActive ? 'text-[--nh-accent]' : 'text-[--sidebar-foreground]'
            }`
          }
        >
