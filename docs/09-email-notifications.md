...
- **Not yet scoped above, requested 2026-07-11 for a future pass:** on new ticket creation (`POST /api/tickets`), also fire an owner-facing alert — an email to `support@mynexusgroup.com` and a Telegram message to William — separate from the counterparty notification in Step 3c above (which notifies the ticket's creator/handler, not Nexus). This mirrors the Chama360 `NS-005` support scaffold's notification pattern (`wsm-second-brain/systems/NS-005-in-app-support-system.md`): best-effort per channel via `Promise.allSettled` (a missing `TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID` or email config must never fail ticket creation), and all user-supplied text HTML-escaped before Telegram interpolation (`parse_mode: HTML` breaks on raw `<`, `>`, `&`). Deliberately deferred — not implemented as part of Phase 08 or this phase's initial scope; needs its own step (mail utility + Telegram bot setup) whenever it's picked up.
