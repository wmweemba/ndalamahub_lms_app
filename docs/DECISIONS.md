...
## Manifi launch decisions (locked 2026-07-22, post-demo planning session with William; product terms confirmed by Clement via WhatsApp)

Inputs to phase docs 19–26. The demo (2026-07-21) succeeded; Manifi confirmed as launch client. Target launch 1 Aug 2026, William can flex +1–3 weeks — build once, correctly.

**Executed** (2026-07-29): production stood up on Coolify per item 8, seeded per item 9, phases 19-27 all complete and merged to `main`. Website intake (item 6) confirmed live end-to-end — a real manifipay.com submission created the test customer/loan used for the launch smoke below. Launch smoke against production: approve, disburse, letter of sale, and prepayment/settlement all done by William directly and confirmed working — catching and fixing a real bug along the way (prepayments never completed a fully-paid loan — see `changelog.md` 2026-07-29). Only the reports/exports smoke check remains before William reports final go/no-go. `server/utils/clearDummyBorrowerData.js` (dry-run by default) is ready to clear the test customer/loan/collateral before Clement's handover. See `docs/PRODUCTION_RUNBOOK.md` for the operational reference and `docs/27-pre-launch-fixes.md`'s close-out for the full pre-handover checklist (backup restore test still outstanding).

1. **Direct lending is a lender-level configuration, not a fork.** `Company.lendingModel: 'employer' | 'direct'`, chosen at lender creation. Manifi is created `direct` in production. Direct customers attach to the lender company itself (`User.company` = the lender; direct loans have `company = lenderCompany`). The employer/payroll model remains fully supported and unchanged for future tenants. `companyLenderId()` resolves a lender-type company to itself (also fixes a discovered fail-open in subscription gating for direct borrowers).
2. **Approval matrix (direct loans):** `lender_officer` and `lender_admin` approve; `lender_admin` disburses (a lean-team admin therefore does both). Employer-side roles have no authority over direct loans. Employer-model loans keep the existing matrix exactly.
3. **Borrower identity is NRC-first.** NRC required for every borrower — replaces Employee ID for direct customers, supplements it for payroll ones (per Clement). NRC unique **per company** (compound partial index), deliberately not global: cross-tenant global uniqueness would leak customer existence between lenders. Email: required+unique for staff roles; **optional for borrowers** (walk-ins without email are never turned away). Borrower username = email if present, else NRC. Credentials: emailed set-password invite when email exists (reuses reset-token infra); staff-set temporary password handed over in person otherwise, with self-service change-password available.
4. **Rollover (Clement's confirmed default mechanic; the "attached image" simulation is authoritative):** 25% flat per 30-day term. Due date passes → 14-day grace window (visibility/flagging only — loan shows overdue/in-arrears, no charge). If any balance is outstanding at due+14: the **outstanding balance** (partial payments reduce it) is capitalized as the new principal, fresh 25% applies, and the **new 30-day term anchors to the previous due date** (issued 1 Aug → K1,000 → K1,250 due 31 Aug → unpaid by 14 Sep → K1,250 principal, K1,562.50 due 30 Sep). Repeats indefinitely; automatic; per-product config (`rollover.enabled`, `graceDays`). No automatic cap — the kill-switch is a human marking the loan `defaulted` (new manual route, lender_admin, reason required), which permanently stops rollover and begins collateral recovery. Loans at 2+ rollovers are surfaced as needing attention. *Open (non-blocking) question with Clement: after how many rollovers does he normally move to selling collateral — tunes the attention threshold only.*
5. **Collateral:** first-class records (separate collection): type ∈ vehicle / business_equipment / title_deed / other(+specify) per Clement, description, estimated value, verification status. **Verification blocks loan approval** (Clement: "blocked until verification"). **A letter of sale on file blocks disbursement** — recorded as a per-collateral flag at launch (paper process; the letter is signed at the shop); document/photo uploads are post-launch. Collateral Register page for lender staff. Product-level `requiresCollateral` (on for Manifi's product).
6. **Website intake (manifipay.com):** applications are open, accounts are gated. Public endpoint `POST /api/public/:slug/applications` — server-side tenant binding via the lender's registered intake config, per-route CORS pinned to the lender's origin, aggressive rate limiting, honeypot, strict validation; no browser-held secret (none can exist). Intake creates a **CustomerApplication (prospect)** — never a User or Loan. Staff review with NRC/phone dedupe surfacing; approval converts atomically to customer + pending loan + declared collateral (or attaches to an existing matched customer). Locked lender subscription = intake refused. No public self-registration, reaffirmed. The website stays vanilla static; its only change is swapping the Submit Sentinel URL for this endpoint per `docs/INTEGRATION_CONTRACT_MANIFIPAY.md`.
7. **Auth: Option A — server-side sessions** (express-session + Mongo store, httpOnly/secure/SameSite-Lax cookies, rolling expiry, per-request user load giving instant deactivation lockout, Origin-check CSRF posture). Replaces localStorage JWTs entirely; refresh route and `JWT_SECRET` retired. **MFA (staff TOTP) is post-launch** — single-tenant reality makes this acceptable; it's the first security enhancement once outreach starts.
8. **Hosting:** production = **Coolify on Hetzner, fresh empty MongoDB** (per the 2026-07-11 cutover decision — now with venue decided). Render + Netlify + Atlas `ndalamahub-prod` remain the **permanent dev/staging/demo environment**, untouched. Client + API served under one parent domain in production so session cookies are first-party.
9. **Launch scope:** single product (25% flat per term, 30-day, rollover on, collateral required); **only Clement's `lender_admin` account seeded** — he creates his team (his choice); production contains zero test residue after the launch smoke. Post-launch backlog (explicitly deferred, not dropped): staff MFA, system-generated loan agreements (Clement: "would be great" — enhancement), collateral document uploads, SMS/WhatsApp notifications, intake-config admin UI, plus the existing UI punch list.
