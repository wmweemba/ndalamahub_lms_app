    ...
    loans.forEach(loan => {
      ...
      upcomingInstallments.forEach(installment => {
        ...
        upcomingPayments.push({
          ...
          borrowerName: loan.applicant ? `${loan.applicant.firstName} ${loan.applicant.lastName}` : 'Deleted user',
          borrowerPhone: loan.applicant ? loan.applicant.phone : '—',
          borrowerEmail: loan.applicant ? loan.applicant.email : '—',
...
router.get('/export', authenticateToken, authorizeMinRole('employer_hr'), async (req, res) => {
  try {
    ...
    const exportData = loans.map(loan => ({
      ...
      applicantEmail: loan.applicant ? loan.applicant.email : '—',
      employeeId: loan.applicant ? loan.applicant.employeeId : '—',
      department: loan.applicant ? loan.applicant.department : '—',
      company: loan.company ? loan.company.name : '—',
      lenderCompany: loan.lenderCompany ? loan.lenderCompany.name : '—',
      amount: loan.amount,
